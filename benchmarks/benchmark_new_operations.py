...
def print_results(name: str, results: dict, n_gaussians: int):
    ...
    print(f"  Mean:       {results['mean_ms']:.3f} ms  ({results['throughput_M_per_sec']:.1f}M/sec)")
...
def main():
    ...
    for n_gaussians in sizes:
        ...
        opacity_op = lambda d: multiply_opacity(d, 0.7, inplace=True)
        ...
        saturation_op = lambda d: Color().saturation(1.3)(d, inplace=True)
        ...
        # Saturation + Vibrance
        sat_vib_op = lambda d: Color().saturation(1.2).vibrance(1.3)(d, inplace=True)
        ...
        sat_hue_op = lambda d: Color().saturation(1.2).hue_shift(15)(d, inplace=True)
        ...
        all_phase2_op = lambda d: Color().saturation(1.2).vibrance(1.1).hue_shift(10).shadows(1.1).highlights(0.9)(d, inplace=True)
        ...
        full_pipeline_op = lambda d: Color().brightness(1.2).contrast(1.1).saturation(1.3).vibrance(1.2).hue_shift(15)(d, inplace=True)
        ...
        print(f"Vibrance overhead vs Saturation: {vibrance_overhead:+.3f} ms ({vibrance_overhead/saturation['mean_ms']*100:+.1f}%)")
        print(f"Hue Shift overhead vs Saturation: {hue_overhead:+.3f} ms ({hue_overhead/saturation['mean_ms']*100:+.1f}%)")
